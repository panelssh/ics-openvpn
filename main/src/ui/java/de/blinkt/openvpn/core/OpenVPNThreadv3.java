...
import net.openvpn.ovpn3.SWIGTYPE_p_std__mapT_std__int32_t_openvpn__DnsServer_std__lessT_std__int32_t_t_t;
import net.openvpn.ovpn3.openvpn_DnsOptions_DomainsList_t;
import net.openvpn.ovpn3.openvpn_DnsOptions_ServersMap_t;
...
public class OpenVPNThreadv3 extends ClientAPI_OpenVPNClient implements Runnable, OpenVPNManagement {
    ...
    public boolean tun_builder_set_dns_options(DnsOptions dns)
    {
        ...
        for (Map.Entry<Integer, DnsServer> dnsServerEntry: sortedDNSServers.entrySet() ) {
            ...
            {
                VpnStatus.logInfo(R.string.dnsserver_ignore_dnnsec, prio, server.to_string());
                continue;
            }
            ...
            {
                VpnStatus.logInfo(R.string.dnsserver_ignore_tls_doh, prio, server.to_string());
                continue;
            }
            ...
            for(DnsAddress address: server.getAddresses())
            {
                ...
                else
                {
                    VpnStatus.logInfo(R.string.dnsserver_ignore_dnsport,
                            address.getAddress(), address.getPort(), prio, server.to_string());
                }
            }
        }
        return dnsadded;
